...
@Component({
  ...
  template: `
    <header class="dark-section z-50 bg-black border-b-2 border-sabotage-border overflow-visible">
      <!-- Mobile Header -->
      <div class="flex md:hidden items-center justify-between px-4 py-4">
        ...
        <div class="flex items-center gap-1">
          ...
          <button
            ...
            (click)="toggleTheme()"
            class="text-sabotage-light p-2 hover:bg-sabotage-gray rounded-lg transition-colors"
            [attr.aria-label]="themeService.isLightMode() ? 'Cambiar a modo oscuro' : 'Cambiar a modo claro'"
          >
      ...
      <div class="hidden md:flex justify-between items-center px-5 py-5">
        ...
        <nav class="flex items-center gap-10 font-space font-bold text-[1.3rem]">
          ...
          <button
            type="button"
            (click)="toggleTheme()"
            class="text-sabotage-light hover:opacity-80 p-2 rounded-full transition-all"
            [attr.aria-label]="themeService.isLightMode() ? 'Cambiar a modo oscuro' : 'Cambiar a modo claro'"
          >
            @if (themeService.isLightMode()) {
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
